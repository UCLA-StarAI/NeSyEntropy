...
class CircuitMPE:
    def __init__(self, vtree_filename, sdd_filename):
        ...
        manager = SddManager(self.vtree)
        self.alpha = io.sdd_read(sdd_filename, manager)

        # Convert to psdd
        pmanager = PSddManager(self.vtree)

        # Storing psdd
        self.beta = pmanager.copy_and_normalize_sdd(self.alpha, self.vtree)
...
if __name__ == '__main__':
    ...
    c = CircuitMPE('test/abcd_constraint.vtree', 'test/abcd_constraint.sdd')
    ...
    # Entropy using KL-divergence
    wmc = c.get_norm_ac(lit_weights)
    kld_entropy = c.entropy_kld()
    ...
    assert(torch.isclose(circuit_entropy, entropy) and torch.isclose(entropy, kld_entropy))

    # Test 2
    vtree = Vtree.read('test/abc.vtree')
    manager = SddManager(vtree)
    alpha = io.sdd_read('test/abc.sdd', manager)

    c = CircuitMPE('test/abc.vtree', 'test/abc.sdd')

    # literal weights are of the form [[-a, a], [-b, b], [-c, c]]
    lit_weights = torch.tensor([[0.7, 0.3], [0.5, 0.5], [0.8, 0.2]], device=torch.cuda.current_device())
    ...
    # Bruteforce Entropy
    m = alpha.models(vtree)
    weights = []
    for model in m:
        pr = 1
        for i in range(3):
            pr *= lit_weights[i][model[i+1]]
        weights.append(pr)
    weights = torch.tensor(weights, device=torch.cuda.current_device())
    probs = weights/wmc
    entropy = -sum([p*log(p) for p in probs])
    ...
    wmc = c.get_tf_ac(lit_weights)
    circuit_entropy = c.Shannon_entropy(lit_weights)
    assert(torch.isclose(kld_entropy, entropy) and torch.isclose(entropy, circuit_entropy))

    # Test 3
    vtree = Vtree.read('test/LKPA_constraint.vtree')
    manager = SddManager(vtree)
    alpha = io.sdd_read('test/LKPA_constraint.sdd', manager)

    c = CircuitMPE('test/LKPA_constraint.vtree', 'test/LKPA_constraint.sdd')
    ...
    # Bruteforce the entropy of the probability distribution
    m = alpha.models(vtree)
    weights = []
    for model in m:
        pr = 1
