...
class NormalizedSddNode(SddNode):
    ...
    def generate_tf_ac(self, litleaves, clear_data=False):
        """ Generates a torch arithmetic circuit according to the weighted model counting procedure for this SDD.
    ...
    def Shannon_entropy(self, litleaves, clear_data=False):
        EPS = torch.tensor(1e-12, device=torch.cuda.current_device())
        for node in self.as_list(clear_data=clear_data):
            ...
            if node.is_false():
                ...
                res = []
                for child in node.positive_elements:
